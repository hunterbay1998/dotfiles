...
export function SysStats() {
  ...
  return (
    <box class="stat-pill" orientation={Gtk.Orientation.HORIZONTAL} spacing={16} halign={Gtk.Align.CENTER}>
      <box orientation={Gtk.Orientation.VERTICAL} halign={Gtk.Align.CENTER} spacing={4}>
        <DotsBar value={() => cpu() / 100} color={[0.97, 0.46, 0.56]} />
        <label label="CPU" class="bar-label" />
        <label label={cpuPct} class="bar-pct cpu-pct" />
      </box>
      <box orientation={Gtk.Orientation.VERTICAL} halign={Gtk.Align.CENTER} spacing={4}>
        <DotsBar value={() => ram() / 100} color={[0.48, 0.64, 0.97]} />
        <label label="RAM" class="bar-label" />
        <label label={ramPct} class="bar-pct ram-pct" />
      </box>
      <box orientation={Gtk.Orientation.VERTICAL} halign={Gtk.Align.CENTER} spacing={4}>
        <DotsBar value={() => gpu() / 100} color={[0.62, 0.81, 0.42]} />
        <label label="GPU" class="bar-label" />
        <label label={gpuPct} class="bar-pct gpu-pct" />
      </box>
    </box>
  )
}
